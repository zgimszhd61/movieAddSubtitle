...
def adjust_font_size(video_width, text, max_width_ratio=0.8, font='STHeiti', initial_fontsize=24, min_fontsize=16):
    ...
    txt_clip = TextClip(txt=text, font=font, fontsize=fontsize, color='white', stroke_color='black', stroke_width=1)
    while txt_clip.size[0] > video_width * max_width_ratio and fontsize > min_fontsize:
        fontsize -= 1
        txt_clip = TextClip(txt=text, font=font, fontsize=fontsize, color='white', stroke_color='black', stroke_width=1)
    return fontsize

def add_subtitles(video_path, subtitles_path, output_path, font='STHeiti', font_color='yellow', stroke_color='black', stroke_width=1, bg_color='black'):
    ...
    for sub in subtitles:
        ...
        if start < video_duration:
            ...
            # 创建带有背景和轮廓的TextClip
            txt_clip_bg = TextClip(txt=sub.text, font=font, fontsize=fontsize, color=font_color, stroke_color=stroke_color, stroke_width=stroke_width, bg_color=bg_color)
